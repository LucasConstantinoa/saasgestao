...
async function testSafeFields() {
  ...
  // Trying WITHOUT total_prepaid_balance
  try {
    const response = await axios.get(`https://graph.facebook.com/v22.0/act_${cleanId}`, {
      params: {
        access_token: token,
        appsecret_proof: proof,
        fields: 'name,balance,funding_source_details{display_string,balance}'
      }
    });
    console.log('FB Safe Response:', JSON.stringify(response.data, null, 2));
  } catch (e) {
    console.error('FB Safe Error:', e.response?.data || e.message);
  }
}
